...
for event in longpoll.listen():
    ...
    if event.type == VkEventType.MESSAGE_NEW:
        ...
        if event.to_me:
            ...
            elif B_d.stage(vk_id) == 5 and request == "Следующий":
                people = B_d.get_people(vk_id)
                write_msg(event.user_id, f"{people}")
                B_d.add_favorite_people(vk_id, people)
            elif B_d.stage(vk_id) == 5 and request == "Не нравится":
                B_d.del_last_favorite_people(vk_id)
                write_msg(event.user_id, f"Введите 'Следующий', если хотите посмотреть следующего человека")
            else:
                write_msg(event.user_id, f"Введите 'Следующий', если хотите посмотреть следующего человека")